...
?>
<center><b><?php echo $this->Html->link($this->Html->image("botoes/seta_esq.png", array("alt" => "Dia anterior", "title" => "Dia anterior")), array('action' => 'index', $data_anterior), array('escape' => false)); ?><font size="4"><u>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $formatter->format($data_extenso); ?></font></u></b>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $this->Html->link($this->Html->image("botoes/seta_dir.png", array("alt" => "Dia posterior", "title" => "Dia posterior")), array('action' => 'index', $data_posterior), array('escape' => false)); ?></center>
<br>
<br>
                                ...
                                <?php if ($key == $item['Event']['especialista_id']) { ?>
                                    <?php if ($hora == substr($item['Event']['start'], 11, 5)) { ?>
                                        ...
                                        <?php if ($minuto_fim == '00') { ?>
                                            <?php $minuto_fim = 59; ?>
                                        <?php } ?>
                                        <?php if (($minuto_fim > $minuto_inicio) and ((int) substr($hora_fim, 0, 2) >= (int) substr($hora, 0, 2))) { ?>
                                        ...
                                        <?php endwhile; ?>
                                    <?php } ?>
                                    <?php if (($minuto_inicio > $minuto_fim) and ((int) substr($hora_fim, 0, 2) >= (int) substr($hora, 0, 2))) { ?>
                                        <?php while ($minuto_fim < ($minuto_inicio - 15)) : ?>
                                            <?php $minuto_fim = $minuto_fim + 15; ?>
                                            <?php $count = $count + 1; ?>
                                            <?php $hora_aux = substr($hora, 0, 2); ?>
                                            <?php $minuto_inicio = 0; ?>
                                            <?php $hora_aux = substr($hora, 0, 2) + 1; ?>
                                            <?php if ($hora_aux < 10) { ?>
                                                <?php $hora_aux = '0' . $hora_aux ?>
                                            <?php } ?>
                                            <tr>
                                                <?php if ($minuto_inicio == 0) { ?>
                                                    <td><b><?php echo $hora_aux . ':' . $minuto_inicio . '0'; ?></b></td>
                                                <?php } else { ?>
                                                    <td><?php echo $hora_aux . ':' . $minuto_inicio; ?></td>
                                                <?php } ?>
                                                <td colspan="1" title='<?php echo 'Tipo de serviço: ' . $item['EventType']['name'] . ' - ' . ' Valor: ' . number_format($item['EventType']['valor'], 2, ",", "") ?>' bgcolor='<?php echo $cor; ?>'><font color="white"><?php echo ''; ?></td></font>
                                            </tr>
                                        <?php endwhile; ?>
                                    <?php } ?>
                                    <?php if (($minuto_inicio > $minuto_fim) and ((int) substr($hora_fim, 0, 2) >= (int) substr($hora, 0, 2))) { ?>
                                        <?php $final = $minuto_inicio / $minuto_fim; ?>
                                        <?php $end = 0; ?>
                                        <?php while ($final > $end) : ?>
                                            <?php $minuto_inicio = $minuto_inicio + 15; ?>
                                            <?php $count = $count + 1; ?>
